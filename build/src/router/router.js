...
/* ------------------------------------------------------------------ */
// Index page
create_router_1.RouterServer.get('/', function (req, res) {
    var params = {
...
// Profile page
create_router_1.RouterServer.get('/profile/:id?', function (req, res) {
    // ПЕРЕДЕЛАТЬ!!!
    // let user = {};
...
// Single product page
create_router_1.RouterServer.get('/product/:id?', function (req, res) {
    if (!req.params.id) {
        res.redirect('/shop');
        return;
    }
    connect_1.pool.query("SELECT * FROM products WHERE id = ?", [req.params.id], function (err, results, fields) {
        if (err) {
            console.error(err);
            res.redirect('/shop');
            return;
        ...
        connect_1.pool.query("SELECT * FROM reviews WHERE review_product_id = ?", [req.params.id], function (error1, reviews, field) {
            if (error1) {
                console.error(error1);
                return;
            }
            connect_1.pool.query('SELECT product_amount FROM purchases WHERE product_id = ?', [req.params.id], function (error2, amounts, fields) {
                if (error2) {
...
// Shop page
create_router_1.RouterServer.get('/shop/:category?/:page?/:priceMin?/:priceMax?/', function (req, res) {
    ...
    connect_1.pool.query(query, function (err, products, field) {
        if (err) {
            console.error(err);
            res.redirect('/');
            return;
...
// Log in page
create_router_1.RouterServer.get('/login/:error?', function (req, res) {
    if (req.cookies.user)
        res.redirect("/");
    var params = {
...
// Sign up page
create_router_1.RouterServer.get('/signup', function (req, res) {
    if (req.cookies.user)
        res.redirect("/");
    var params = {
...
// Logout logic
create_router_1.RouterServer.get('/logout', function (req, res) {
    res.clearCookie("user");
    res.redirect('/');
});
// Search logic
create_router_1.RouterServer.get('/search/:query?', function (req, res) {
    ...
    query_1.db.getAll(query, [], function (searchFields) {
